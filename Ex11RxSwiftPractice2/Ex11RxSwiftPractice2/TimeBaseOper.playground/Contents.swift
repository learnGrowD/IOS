...
//            ,count: 만들어낼최대Observavble,
//            scheduler: MainScheduler.instance
//    )
//    .flatMap { windowObservable -> Observable<indext : in, element : String> in
//
//    }
...
print("-----timeout-----")
let 누리지않으면에러 = UIButton(type: .system)
누리지않으면에러.setTitle("눌러주세용", for: .normal)
누리지않으면에러.sizeToFit()

PlaygroundPage.current.liveView = 누리지않으면에러
누리지않으면에러.rx.tap
    .do(onNext: {
      print("tap")
    })
